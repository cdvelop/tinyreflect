...
> IMPORTANT: This repository is archived. The standard `reflect` implementation shipped with TinyGo v0.39.0 now provides acceptable functionality and binary size for most use cases, so this project is no longer actively maintained.

> For TinyGo v0.39.0 and later, prefer using the built-in `reflect` unless you have a very specific, size-constrained requirement. This archive notice explains the reason and points users toward the standard library for most scenarios.
